...
#if BUILDFLAG(IS_IOS_TVOS) && BUILDFLAG(USE_STARBOARD_MEDIA)
  // Check platform-specific init data types registered at runtime by internal
  // code (e.g. URL player DRM types).
  media::EmeInitDataType platform_type =
      media::PlatformInitDataTypes::ToEnum(init_data_type.Utf8());
  if (platform_type != media::EmeInitDataType::UNKNOWN) {
  ...
  switch (init_data_type) {
    ...
    case media::EmeInitDataType::PLATFORM_DRM: {
      const std::string& platform_str =
          media::PlatformInitDataTypes::ToString(init_data_type);
      if (!platform_str.empty()) {
        return String::FromUTF8(platform_str);
      }
      return String();
    }
#endif  // BUILDFLAG(IS_IOS_TVOS) && BUILDFLAG(USE_STARBOARD_MEDIA)

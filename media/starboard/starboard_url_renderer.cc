...
#include "media/starboard/starboard_url_renderer.h"
...
StarboardUrlRenderer::StarboardUrlRenderer(
...
void StarboardUrlRenderer::Initialize(MediaResource* media_resource,
  ...
  TRACE_EVENT0("media", "StarboardUrlRenderer::Initialize");
  ...
  if (suspended_) {
    task_runner_->PostDelayedTask(
        FROM_HERE,
        base::Bind(&StarboardUrlRenderer::Initialize, this, media_resource,
                   client, init_cb),
  ...
  if (!source_url_.empty()) {
    // URL player (AVPlayer) handles stream discovery, demuxing, and decoding
    // natively. Skip stream checks and go straight to player creation.
    LOG(INFO) << "URL player path: skipping stream checks for " << source_url_;
...
void StarboardUrlRenderer::SetCdm(CdmContext* cdm_context,
                                  CdmAttachedCB cdm_attached_cb) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  DCHECK(cdm_context);
  TRACE_EVENT0("media", "StarboardUrlRenderer::SetCdm");
...
void StarboardUrlRenderer::Flush(base::OnceClosure flush_cb) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  ...
  if (buffering_state_ != BUFFERING_HAVE_NOTHING) {
    ...
    task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&StarboardUrlRenderer::OnBufferingStateChange,
...
void StarboardUrlRenderer::StartPlayingFrom(TimeDelta time) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  DCHECK_EQ(buffering_state_, BUFFERING_HAVE_NOTHING);

  LOG(INFO) << "StarboardUrlRenderer::StartPlayingFrom() called with " << time
            << '.';
  ...
  if (audio_read_in_progress_ || video_read_in_progress_) {
    ...
    task_runner_->PostDelayedTask(
        FROM_HERE,
        base::BindOnce(&StarboardUrlRenderer::StartPlayingFrom,
...
void StarboardUrlRenderer::SetPlaybackRate(double playback_rate) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  if (playback_rate < 0.0) {
    LOG(INFO)
        << "StarboardUrlRenderer::SetPlaybackRate(): invalid playback rate "
        << playback_rate << '.';
    return;
  }
...
void StarboardUrlRenderer::SetVolume(float volume) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());

  if (volume < 0.0 || volume > 1.0) {
    LOG(INFO) << "StarboardUrlRenderer::SetVolume(): invalid volume " << volume
              << '.';
    return;
  }
...
TimeDelta StarboardUrlRenderer::GetMediaTime() {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  ...
  if (statistics.video_frames_decoded > 0 ||
    ...
    task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&StarboardUrlRenderer::OnStatisticsUpdate,
...
void StarboardUrlRenderer::OnTracksChanged(
...
void StarboardUrlRenderer::SetStarboardUrlRendererCallbacks(
...
void StarboardUrlRenderer::OnVideoGeometryChange(const gfx::Rect& output_rect) {
...
void StarboardUrlRenderer::OnSbWindowHandleReady(
...
void StarboardUrlRenderer::SetSourceUrl(const std::string& source_url) {
  CHECK(task_runner_->RunsTasksInCurrentSequence());
  source_url_ = source_url;
  LOG(INFO) << "StarboardUrlRenderer::SetSourceUrl: " << source_url;
}

#if SB_HAS(PLAYER_WITH_URL)
void StarboardUrlRenderer::OnEncryptedMediaInitDataEncountered(
...
#if BUILDFLAG(IS_ANDROID)
void StarboardUrlRenderer::OnOverlayInfoChanged(
  ...
  config.ready_cb = base::BindOnce(&StarboardUrlRenderer::OnOverlayReady,
                                   weak_factory_.GetWeakPtr());
  config.failed_cb = base::BindOnce(&StarboardUrlRenderer::OnOverlayFailed,
                                    weak_factory_.GetWeakPtr());
...
SbPlayerInterface* StarboardUrlRenderer::GetSbPlayerInterface() {
...
void StarboardUrlRenderer::CreatePlayerBridge() {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  DCHECK(init_cb_);
  DCHECK_EQ(state_, STATE_INITIALIZING);
  DCHECK(audio_stream_ || video_stream_ || !source_url_.empty());

  TRACE_EVENT0("media", "StarboardUrlRenderer::CreatePlayerBridge");
  ...
  if (suspended_) {
    task_runner_->PostDelayedTask(
        FROM_HERE,
        base::Bind(&StarboardUrlRenderer::CreatePlayerBridge, this, init_cb),
        base::Milliseconds(kRetryDelayAtSuspendInMilliseconds));
    return;
  }
#endif  // BUILDFLAG(COBALT_MEDIA_ENABLE_SUSPEND_RESUME)
...
#if SB_HAS(PLAYER_WITH_URL)
  // TODO: TEMPORARY - Hardcode HLS URL to test AVPlayer flow end-to-end.
  if (source_url_.empty()) {
    source_url_ = "https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8";
  ...
  if (!source_url_.empty()) {
    ...
    player_bridge_.reset(new SbPlayerBridge(
        ...
        base::BindRepeating(
            &StarboardUrlRenderer::OnEncryptedMediaInitDataEncountered,
...
#endif  // SB_HAS(PLAYER_WITH_URL)
  LOG(INFO) << "CreatePlayerBridge: checking IsValid()...";
  if (player_bridge_->IsValid()) {
#if SB_HAS(PLAYER_WITH_URL)
    if (!source_url_.empty()) {
      // URL player: skip audio config query — AVPlayer manages audio natively.
      LOG(INFO) << "URL-based SbPlayerBridge created successfully.";
    } else {
#endif  // SB_HAS(PLAYER_WITH_URL)
  ...
  LOG(INFO) << "StarboardUrlRenderer::CreatePlayerBridge() failed to create a"
  ...
  std::move(init_cb_).Run(PipelineStatus(
      ...
      "StarboardUrlRenderer::CreatePlayerBridge() failed to create a valid"
      " SbPlayerBridge - \"" +
          error_message + "\""));
}

void StarboardUrlRenderer::ApplyPendingBounds() {
...
void StarboardUrlRenderer::UpdateDecoderConfig(DemuxerStream* stream) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void StarboardUrlRenderer::OnDemuxerStreamRead(
  ...
  if (!task_runner_->RunsTasksInCurrentSequence()) {
    task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&StarboardUrlRenderer::OnDemuxerStreamRead,
  ...
  if (status == DemuxerStream::kOk) {
    ...
    stream->Read(
        max_buffers,
        base::BindOnce(&StarboardUrlRenderer::OnDemuxerStreamRead,
...
void StarboardUrlRenderer::OnStatisticsUpdate(const PipelineStatistics& stats) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  client_->OnStatisticsUpdate(stats);
}

void StarboardUrlRenderer::OnNeedData(DemuxerStream::Type type,
  ...
  if (!player_bridge_) {
    LOG(INFO) << "StarboardUrlRenderer::OnNeedData() called with " << type;
    return;
  }
  ...
  if (type == DemuxerStream::AUDIO) {
    ...
    if (!is_video_eos_written_ && time_ahead_of_playback_for_preroll >
      ...
      if (time_ahead_of_playback >
        ...
        task_runner_->PostDelayedTask(
            FROM_HERE,
            base::BindOnce(&StarboardUrlRenderer::DelayedNeedData,
...
void StarboardUrlRenderer::OnPlayerStatus(SbPlayerState state) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());

  LOG(INFO) << "StarboardUrlRenderer::OnPlayerStatus() called with "
  ...
  if (!player_bridge_) {
    LOG(WARNING)
        << "StarboardUrlRenderer::OnPlayerStatus() called with invalid "
           "SbPlayerBridge";
    return;
  }
  ...
  switch (state) {
    ...
    case kSbPlayerStatePresenting:
      ...
      if (source_url_.empty()) {
#endif  // SB_HAS(PLAYER_WITH_URL)
        LOG(INFO) << "OnPlayerStatus: calling GetAudioConfigurations...";
        audio_write_duration_for_preroll_ = audio_write_duration_ =
            HasRemoteAudioOutputs(player_bridge_->GetAudioConfigurations())
                ? audio_write_duration_remote_
                : audio_write_duration_local_;
        ...
        LOG(INFO) << "OnPlayerStatus: SKIPPING GetAudioConfigurations "
                  << "(URL player)";
        // Report video dimensions so the web layer creates a video hole
        // for punch-out mode rendering.
        int width = 0, height = 0;
        player_bridge_->GetVideoResolution(&width, &height);
        if (width > 0 && height > 0) {
          gfx::Size natural_size(width, height);
          LOG(INFO) << "[URL-ROUTING] URL player reporting natural size: "
                    << natural_size.ToString();
          client_->OnVideoNaturalSizeChange(natural_size);
          paint_video_hole_frame_cb_.Run(natural_size);
        } else {
          gfx::Size default_size(1920, 1080);
          LOG(INFO) << "[URL-ROUTING] URL player using default size: "
...
void StarboardUrlRenderer::OnPlayerError(SbPlayerError error,
                                         const std::string& message) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  LOG(ERROR) << "StarboardUrlRenderer::OnPlayerError() called with code "
             << error << " and message \"" << message << "\"";
...
void StarboardUrlRenderer::NotifyError(PipelineStatus status) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void StarboardUrlRenderer::DelayedNeedData(int max_number_of_buffers_to_write) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void StarboardUrlRenderer::StoreMediaTime(TimeDelta media_time) {
  last_media_time_ = media_time;
  last_time_media_time_retrieved_ = Time::Now();
}

#if BUILDFLAG(IS_ANDROID)
void StarboardUrlRenderer::OnOverlayReady(AndroidOverlay* overlay) {
...
void StarboardUrlRenderer::OnOverlayFailed(AndroidOverlay* overlay) {
  DCHECK_EQ(overlay, overlay_.get());
  overlay_ = nullptr;
  state_ = STATE_ERROR;
  if (init_cb_) {
    std::move(init_cb_).Run(PipelineStatus(
        DECODER_ERROR_NOT_SUPPORTED,
        "StarboardUrlRenderer::OnOverlayFailed() failed to create a "
        "valid AndroidOverlay"));
  }
}
#endif  // BUILDFLAG(IS_ANDROID)

int StarboardUrlRenderer::GetDefaultMaxBuffers(AudioCodec codec,
...
int StarboardUrlRenderer::GetEstimatedMaxBuffers(
    TimeDelta write_duration,
    TimeDelta time_ahead_of_playback,
    bool is_preroll) {
...
void StarboardUrlRenderer::OnBufferingStateChange(
...
SbDecodeTarget StarboardUrlRenderer::GetSbDecodeTarget() {
...
void StarboardUrlRenderer::set_decode_target_graphics_context_provider(
